...
import 'package:markets_deliveryboy/src/repository/market_repository.dart';
...
class AllOrdersController extends ControllerMVC {
  List<Order> orders = <Order>[];
  GlobalKey<ScaffoldState> scaffoldKey;
  ...
  void listenForAllOrders({String message}) async {
    final Stream<Order> stream = await getAllOrders();
    stream.listen((Order _order) {
      setState(() {
        orders.add(_order);
      });
  ...
  Future<void> refreshOrders() async {
    orders.clear();
    listenForAllOrders(message: S.of(context).order_refreshed_successfuly);
  }
}
