...
class OrderDetailsController extends ControllerMVC {
  ...
  void doDeliveredOrder(Order _order, String code, String spendAmount) async {
    deliveredOrder(_order, code, spendAmount).then((value) {
      if (value['status']) {
        setState(() {
  ...
  void doCancelOrder(Order _order) async {
    cancelledOrder(_order).then((value) {
      if (value['status']) {
        setState(() {
          this.order.orderStatus.id = '0';
        });
      }
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: Text(value['message']),
